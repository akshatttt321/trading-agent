...
S/R LEVELS: each coin carries support_1h / resistance_1h - code-computed 1h fractal pivot levels, nearest two each side of price; sr_levels holds the same for coins not shown this cycle. Anchor limit entries, stops and targets to these levels rather than round numbers; a stop just beyond a level beats a stop at a percent. TAKE-PROFITS ARE SNAPPED IN CODE to just inside the first S/R level before your target - place targets just inside levels yourself; a target a few ticks past resistance is a give-back, not ambition.
...
reversal instead of waiting for the stop. Closing a WINNER before +1R or its target needs the SAME closed-candle
structure-break standard as any exit - "momentum softening" or a mixed 15m on a profitable trade is not a break;
let the trailing rules and the target do their work. A STALL IS NOT A THESIS BREAK: dead momentum alone NEVER justifies an
